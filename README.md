...
- 从 FCB 页面执行“笔记 + 字幕”导入时，插件只接受有明确页面证据的对应视频；无法精确关联时会停止，不会猜测其他标签页中的视频。

## 导入诊断

如导入失败或结果不完整，打开命令面板并执行 **复制最近一次百度导入诊断信息**。报告包含失败阶段、字幕来源、候选数量和重试结果，不包含完整网址、Cookie、登录令牌或百度会话标识。
